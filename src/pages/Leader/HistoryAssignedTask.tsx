...
import { getLeaderAssignmentHistory } from "../../services/leader-api";
      ...
      <table className="w-full text-sm text-left text-gray-500 mt-4">
        <thead className="text-xs text-gray-700 uppercase bg-gray-50 ">
          <tr>
            ...
            <th scope="col" className="px-6 py-3">
              Reject Reason / <br /> Action Reason
            </th>
            <th scope="col" className="px-6 py-3">
              Status
            </th>
            ...
            data.data.map((item) =>
              item.task ? (
                <tr className="bg-white border-b hover:bg-gray-50">
                  <td className="px-6 py-4">{item.task.taskId}</td>
                  <td className="px-6 py-4">{item.task.vulnerabilityId}</td>
                  <td className="px-6 py-4">
                    {item.task.vulnerability.vulnerabilityName}
                  </td>
                  <td className="px-6 py-4">{item.task.assignedToName}</td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    {item.task.fixedAt
                      ? new Date(item.task.fixedAt).toLocaleDateString(
                          "en-GB",
                          {
                            day: "2-digit",
                            month: "2-digit",
