...
const GanttChart = ({ id }: { id: string | undefined }) => {
  ...
  if (vulnerabilities.length === 0) {
    return <div>No data available for vulnerabilities.</div>;
  }
  ...
  const tasks: CustomTask[] = vulnerabilities.map((vul: any) => ({
    ...
    progress: 50, // Chuyển đổi status thành progress (giả định nếu statusId là 8 thì progress = 50)
  ...
  const CustomTooltip = ({ task }: { task: CustomTask }) => {
    return (
      ...
      >
        ...
        <p>
          Time: {task.start.toLocaleDateString()} -{" "}
